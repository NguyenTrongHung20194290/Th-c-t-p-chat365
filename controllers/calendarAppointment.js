...
export const editCalendar = async (req, res, next) => {
    try {
        if (req && req.body) {
            ...
            if (saveCalendar) {
                const result = { ...saveCalendar }
                result._doc.time = formatDate(createTime, typeDate)
                res.json({
                    data: {
                        result: result._doc,
                        message: 'Success'
...
export const getDetailCalendar = async (req, res, next) => {
    try {
        if (req && req.params) {
            ...
            const calendar = await CalendarAppointment.findOne({ _id: calendarId })
            const user = await User.findOne({ _id: Number(calendar.senderId) }, { userName: 1, avatarUser: 1 })
            if (calendar && user) {
                const result = { ...calendar }
                result._doc.senderName = user.userName
                if (result._doc.avatarUser !== '') {
                    result._doc.avatarUser = `https://mess.timviec365.vn/avatarUser/${result._doc._id}/${result._doc.avatarUser}`
                }
                else {
                    result._doc.avatarUser = `https://mess.timviec365.vn/avatar/${result._doc.userName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                }
...
export const getAllCalendarOfUser = async (req, res, next) => {
    try {
        if (req && req.body) {
            ...
            const calendar = await CalendarAppointment.find({ participant: { $regex: userId } }).sort({ createTime: 'desc' })
            if (calendar) {
                res.json({
                    data: {
                        result: calendar,
